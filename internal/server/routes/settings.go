...
func (v *ViewRoutes) handleSettingsUpdate(c echo.Context) error {
	...
	update := appservices.OrganizationSettingsUpdate{
		...
		RequiredFields:              make([]appservices.RequiredFieldInput, 0, len(payload.RequiredFields)),
	}
	for _, field := range payload.RequiredFields {
		update.RequiredFields = append(update.RequiredFields, appservices.RequiredFieldInput{
			Label:      field.Label,
			Type:       field.Type,
