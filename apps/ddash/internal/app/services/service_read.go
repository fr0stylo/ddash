...
func (s *ServiceReadService) GetServiceDetail(ctx context.Context, organizationID int64, name string) (domain.ServiceDetail, error) {
	...
	dependencies, err := s.serviceStore.ListServiceDependencies(ctx, organizationID, name)
	if err != nil {
		return domain.ServiceDetail{}, err
	}
	dependants, err := s.serviceStore.ListServiceDependants(ctx, organizationID, name)
	if err != nil {
		return domain.ServiceDetail{}, err
	}
...
// UpsertServiceDependency creates or keeps an existing dependency relation.
func (s *ServiceReadService) UpsertServiceDependency(ctx context.Context, organizationID int64, serviceName, dependsOnServiceName string) error {
	serviceName = strings.TrimSpace(serviceName)
	dependsOnServiceName = strings.TrimSpace(dependsOnServiceName)
	if serviceName == "" || dependsOnServiceName == "" {
		return nil
	}
	if strings.EqualFold(serviceName, dependsOnServiceName) {
		return nil
	}
	return s.serviceStore.UpsertServiceDependency(ctx, organizationID, serviceName, dependsOnServiceName)
}

// DeleteServiceDependency removes a dependency relation.
func (s *ServiceReadService) DeleteServiceDependency(ctx context.Context, organizationID int64, serviceName, dependsOnServiceName string) error {
	serviceName = strings.TrimSpace(serviceName)
	dependsOnServiceName = strings.TrimSpace(dependsOnServiceName)
	if serviceName == "" || dependsOnServiceName == "" {
		return nil
	}
	return s.serviceStore.DeleteServiceDependency(ctx, organizationID, serviceName, dependsOnServiceName)
}
