...
type orgRouteStoreFake struct {
	...
	deletedInstall  int64
	setupIntents    map[string]ports.GitHubSetupIntent
}
...
func (f *orgRouteStoreFake) UpsertGitHubInstallationMapping(context.Context, ports.GitHubInstallationMapping) error {
	return nil
}

func (f *orgRouteStoreFake) ListGitHubInstallationMappings(context.Context, int64) ([]ports.GitHubInstallationMapping, error) {
	return f.githubMappings, nil
}

func (f *orgRouteStoreFake) DeleteGitHubInstallationMapping(_ context.Context, installationID, _ int64) error {
	f.deletedInstall = installationID
	return nil
}

func (f *orgRouteStoreFake) GetOrganizationByGitHubInstallationID(context.Context, int64) (ports.Organization, error) {
	return ports.Organization{}, sql.ErrNoRows
}

func (f *orgRouteStoreFake) CreateGitHubSetupIntent(_ context.Context, intent ports.GitHubSetupIntent) error {
	if f.setupIntents == nil {
		f.setupIntents = map[string]ports.GitHubSetupIntent{}
	}
	f.setupIntents[intent.State] = intent
	return nil
}

func (f *orgRouteStoreFake) GetGitHubSetupIntentByState(_ context.Context, state string) (ports.GitHubSetupIntent, error) {
	if f.setupIntents == nil {
		return ports.GitHubSetupIntent{}, sql.ErrNoRows
	}
	intent, ok := f.setupIntents[state]
	if !ok {
		return ports.GitHubSetupIntent{}, sql.ErrNoRows
	}
	return intent, nil
...
func (f *orgRouteStoreFake) DeleteGitHubSetupIntent(_ context.Context, state string) error {
	if f.setupIntents != nil {
		delete(f.setupIntents, state)
	}
	return nil
}
